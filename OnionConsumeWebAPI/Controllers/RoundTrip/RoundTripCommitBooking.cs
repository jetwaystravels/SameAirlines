...
namespace OnionConsumeWebAPI.Controllers.RoundTrip
{
    public class RoundTripCommitBooking : Controller
    {
		...
		public async Task<IActionResult> RoundTripBookingView()
        {
            ...
            {
                string Leftdata = data.Airline[0].ToString();
                ...
                for (int k1 = 0; k1 < data.Airline.Count; k1++)
                {
                    ...
                    if (!string.IsNullOrEmpty(tokenview) && flagAirAsia == true && data.Airline[k1].ToLower().Contains("akasaair"))
                    {
                        ...
                        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                        HttpResponseMessage responceCommit_Booking = await client.PostAsJsonAsync(AppUrlConstant.AkasaCommitBooking, _Commit_BookingModel);
                        ...
                        || data.Airline[k1].ToLower().Contains("batik") || data.Airline[k1].ToLower().Contains("omanair") || data.Airline[k1].ToLower().Contains("saudia")))
                    {
                        ...
                        if (!string.IsNullOrEmpty(tokenview))
                        {
                            ...
                            using (HttpClient client1 = new HttpClient())
                            {
                                ...
                                RecordLocator = Regex.Match(res, @"universal:ProviderReservationInfo[\s\S]*?LocatorCode=""(?<LocatorCode>[\s\S]*?)""", RegexOptions.IgnoreCase | RegexOptions.Multiline).Groups["LocatorCode"].Value.Trim();
                                ...
                                if (!string.IsNullOrEmpty(strResponse) && !string.IsNullOrEmpty(RecordLocator))
                                {
                                    ...
                                    if (pnrResDetail != null)
                                    {
                                        ...
                                        if (_unitdesinator.unitDesignatorPax != null)
                                            _contact.ReturnPaxSeats = "";// _unitdesinator.unitDesignatorPax.ToString();
                                        returnTicketBooking.airLines = pnrResDetail.Bonds.Legs[0].AirlineName;
