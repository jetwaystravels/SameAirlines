...
namespace OnionArchitectureAPI.Services.Travelport
{
    public class TravelPortParsing
    {
        ...
        public List<GDSResModel.Segment> ParseLowFareSearchRsp2_pricepoint(string lowFareResponse, string contractType_, DateTime depDate)
        {
            List<GDSResModel.Segment> listOfSegment = new List<GDSResModel.Segment>();
            //ArrayList listofTPSegment = null;
            ArrayList listOfBound = new ArrayList();
            ArrayList listOfOutBound = new ArrayList();
            ArrayList listOfInBound = new ArrayList();
            int contractId = 1;
            string TPtransactionId = string.Empty;
            string outBoundGroup = string.Empty;
            string inBoundGroup = string.Empty;
                        ...
                        if (root.Name.Equals("SOAP:Body", StringComparison.OrdinalIgnoreCase))
                        {
                            foreach (XmlNode lowFareSearchRes in root)// LOOP FOR SOAPBODY GOT 1 NODE AS LOWFARESEARCHRSP (LOWFARESEARCHRSP)
                            {
                                if (lowFareSearchRes.Name.Equals("air:LowFareSearchRsp", StringComparison.OrdinalIgnoreCase))
                                {
                                    ...
                                    foreach (XmlNode airPricingSolution in lowFareSearchRes) // LOOP HERE FOR lOWFARESEARCHrSP 6 
                                    {
                                        ...
                                        #region AirPricingSolution
                                        if (airPricingSolution.Name.Equals("air:AirPricePointList", StringComparison.OrdinalIgnoreCase))
                                        {
                                            //Main
                                            foreach (XmlNode airPricingpoint in airPricingSolution) //for all pricePoint
                                            {

                                                if (airPricingpoint.Name.Equals("air:AirPricePoint", StringComparison.OrdinalIgnoreCase))
                                                {
                                                    fare = new GDSResModel.Fare();
                                                    fare.PaxFares = new List<PaxFare>();
                                                    ...
                                                    decimal TMarkup = 0;
                                                    if (airPricingpoint.Attributes["TotalPrice"].Value.Contains("INR"))
                                                    {
                                                        fare.TotalFareWithOutMarkUp = Convert.ToDecimal(airPricingpoint.Attributes["TotalPrice"].Value.Remove(0, 3));
                                                    ...
                                                    {
                                                        fare.TotalFareWithOutMarkUp = Convert.ToDecimal(airPricingpoint.Attributes["ApproximateTotalPrice"].Value.Remove(0, 3));
                                                    }
                                                    if (airPricingpoint.Attributes["BasePrice"].Value.Contains("INR"))
                                                    {
                                                        fare.BasicFare = Convert.ToDecimal(airPricingpoint.Attributes["BasePrice"].Value.Remove(0, 3));
                                                    }
                                                    else
                                                    {
                                                        fare.BasicFare = Convert.ToDecimal(airPricingpoint.Attributes["ApproximateBasePrice"].Value.Remove(0, 3));
                                                    }

                                                    if (airPricingpoint.Attributes["Taxes"].Value.Contains("INR"))
                                                    {
                                                        ...
                                                        fare.TotalTaxWithOutMarkUp = Convert.ToDecimal(airPricingpoint.Attributes["Taxes"].Value.Remove(0, 3)) + TMarkup;

                                                    }
                                                    else
                                                    {
                                                        //TMarkup = GetMarkup(Convert.ToDecimal(airPricingSolution.Attributes["Taxes"].Value.Remove(0, 3)), Convert.ToDecimal(airPricingSolution.Attributes["Taxes"].Value.Remove(0, 3)), Convert.ToDecimal(airPricingSolution.Attributes["Taxes"].Value.Remove(0, 3)));
                                                        fare.TotalTaxWithOutMarkUp = Convert.ToDecimal(airPricingpoint.Attributes["ApproximateTaxes"].Value.Remove(0, 3)) + TMarkup;
                                                    }

                                                }
                                                int _airPricingpointCounter = 1;
                                                foreach (XmlNode AirPricingInfo in airPricingpoint) // airpriceinfo 3 times
                                                {
                                                    segmentid = string.Empty;
                                                    paxFare = new GDSResModel.PaxFare();
                                                    paxFare.Fare = new List<GDSResModel.FareDetail>();
                                                    //if (_airPricingpointCounter == airPricingpoint.ChildNodes.Count)
                                                    //{
                                                    if (AirPricingInfo.Name.Equals("air:AirPricingInfo", StringComparison.OrdinalIgnoreCase))
                                                    {
                                                        if (AirPricingInfo.Attributes["Refundable"] != null)
                                                        {
                                                            paxFare.Refundable = bool.Parse(AirPricingInfo.Attributes["Refundable"].Value);
                                                        }

                                                        foreach (XmlNode _AirPricingInfo in AirPricingInfo)
                                                        {
                                                            #region PaxType
                                                            if (_AirPricingInfo.Name.Equals("air:PassengerType", StringComparison.OrdinalIgnoreCase))
                                                            {
                                                                switch (_AirPricingInfo.Attributes["Code"].Value)
                                                                {
                                                                    case "ADT":
                                                                        if (AirPricingInfo.Attributes["BasePrice"].Value.Contains("INR"))
                                                                        {
                                                                            paxFare.BasicFare = Convert.ToDecimal(AirPricingInfo.Attributes["BasePrice"].Value.Remove(0, 3));
                                                                        }
                                                                        else
                                                                        {
                                                                            paxFare.BasicFare = Convert.ToDecimal(AirPricingInfo.Attributes["ApproximateBasePrice"].Value.Remove(0, 3));
                                                                        }
                                                                        paxFare.TotalTax = Convert.ToDecimal(AirPricingInfo.Attributes["Taxes"].Value.Remove(0, 3));
                                                            ...
                                                            #endregion
                                                            #region Change Penality
                                                            if (_AirPricingInfo.Name.Equals("air:ChangePenalty", StringComparison.OrdinalIgnoreCase))
                                                            {
                                                                foreach (XmlNode changePenalty in _AirPricingInfo)
                                                                {
                                                                    if (changePenalty.Name.Equals("air:Amount", StringComparison.OrdinalIgnoreCase))
                                                                    {
                                                            ...
                                                            #endregion
                                                            #region Cancel Penality
                                                            if (_AirPricingInfo.Name.Equals("air:CancelPenalty", StringComparison.OrdinalIgnoreCase))
                                                            {
                                                                foreach (XmlNode cancelPenalty in _AirPricingInfo)
                                                                {
                                                                    if (cancelPenalty.Name.Equals("air:Amount", StringComparison.OrdinalIgnoreCase))
                                                                    {
                                                            ...
                                                            #region Taxes
                                                            if (_AirPricingInfo.Name.Equals("air:TaxInfo", StringComparison.OrdinalIgnoreCase))
                                                            {
                                                                if (_AirPricingInfo.Attributes["Category"] != null && _AirPricingInfo.Attributes["Amount"] != null)
                                                                {
                                                                    fareDetails = new GDSResModel.FareDetail();
                                                                    fareDetails.Amount = decimal.Parse(_AirPricingInfo.Attributes["Amount"].Value.Remove(0, 3));
                                                                    switch (_AirPricingInfo.Attributes["Category"].Value)
                                                                    {
                                                                        case "IN":
                                                                            fareDetails.ChargeCode = "UDF";
                                                                            fareDetails.ChargeDetail = "USER DEVELOPMENT FEE";
                                                                        ...
                                                                        case "YR":
                                                                            ...
                                                                            break;
                                                                        default:
                                                                            fareDetails.ChargeCode = _AirPricingInfo.Attributes["Category"].Value;
                                                                            fareDetails.ChargeDetail = _AirPricingInfo.Attributes["Category"].Value;
                                                                            break;
                                                                    }
                                                                    paxFare.Fare.Add(fareDetails);
                                                                }
                                                            ...
                                                            #endregion
                                                            #region FareInfoRef
                                                            if (_AirPricingInfo.Name.Equals("air:FareInfoRef", StringComparison.OrdinalIgnoreCase))
                                                            {
                                                                if (FareInfoList != null && !string.IsNullOrEmpty(FareInfoList.InnerXml))
                                                                {
                                                                    foreach (XmlNode fareInfoList in FareInfoList) //Loop for xml stored in fareinfolist
                                                                    {
                                                                        if (fareInfoList.Name.Equals("air:FareInfoList", StringComparison.OrdinalIgnoreCase))
                                                                        {
                                                                            foreach (XmlNode fareInfo in fareInfoList)
                                                                            {
                                                                                if (fareInfo.Attributes["Key"].Value.Equals(_AirPricingInfo.Attributes["Key"].Value, StringComparison.Ordinal))
                                                                                {
                                                                                    //GetBrandDetails
                                                                                    ...
                                                                                    try
                                                                                    {
                                                                                        ...
                                                                                        foreach (XmlNode BrandInfo in fareInfo) // loop for air:FareInfo 
                                                                                        {
                                                                                            if (BrandInfo.Name.Equals("air:Brand", StringComparison.OrdinalIgnoreCase))
                                                                                            {
                                                                                                if (BrandInfo.Attributes["BrandID"] != null)
                                                                                                {
                                                                                                    ...
                                                                                                    if (BrandList != null)
                                                                                                    {
                                                                                                        ...
                                                                                                        {
                                                                                                            foreach (XmlNode brandLists in BrandList) //brandlist
                                                                                                            {
                                                            ...
                                                            #endregion
                                                            #region FlightOptionsList
                                                            if (_AirPricingInfo.Name.Equals("air:FlightOptionsList", StringComparison.OrdinalIgnoreCase))
                                                            {
                                                                foreach (XmlNode FlightOption in _AirPricingInfo) //FlightOptionsList 2 flightoption DEL-BOM BOM-del
                                                                {
                                                                    ...
                                                                    airSegstatus = false;
                                                                    _journeyTime = null;
                                                                    if (FlightOption.Name.Equals("air:FlightOption", StringComparison.OrdinalIgnoreCase))
                                                                    {
                                                                        foreach (XmlNode BookingInfo in FlightOption)// flight option
                                                                        {
                                                                            bond = new GDSResModel.Bond();
                                                                            //leg = new GDSResModel.Leg();
                                                                            bond.Legs = new List<GDSResModel.Leg>();
                                                                            ...
                                                                            //listOfInBound to do
                                                                            foreach (XmlNode _BookingInfo in BookingInfo)
                                                                            {
                                                                                if (_BookingInfo.Name.Equals("air:Connection", StringComparison.OrdinalIgnoreCase))
                                                                                {
                                                                                    continue;
                                                                                }
                                                                                if (_BookingInfo.Name.Equals("air:BookingInfo", StringComparison.OrdinalIgnoreCase))
                                                                                {
                                                                                    leg = new GDSResModel.Leg();
                                                                                    if (airSegmentList != null && !string.IsNullOrEmpty(airSegmentList.InnerXml))
                                                                                    {
                                                                                            ...
                                                                                            if (airSegmentlist.Name.Equals("air:AirSegmentList", StringComparison.OrdinalIgnoreCase))
                                                                                            {
                                                                                                foreach (XmlNode airSegment in airSegmentlist)
                                                                                                {
                                                                                                    if (airSegment.Name.Equals("air:AirSegment", StringComparison.OrdinalIgnoreCase))
                                                                                                    {
                                                                                                        ...
                                                                                                        if (airSegment.Attributes["Key"].Value.Equals(_BookingInfo.Attributes["SegmentRef"].Value, StringComparison.Ordinal))
                                                                                                        {
                                                                                                            ...
                                                                                                            leg.Duration = airSegment.Attributes["FlightTime"].Value;
                                                                                                            leg.AircraftCode = _BookingInfo.Attributes["SegmentRef"].InnerText;
                                                                                                            if (flightnumber == "")
                                                                                                            {
                                                                                                                            ...
                                                                                                                            foreach (XmlNode flightDetaillist_ in flightDetailsList)
                                                                                                                            {
                                                                                                                                foreach (XmlNode flightDetails in flightDetaillist_)
                                                                                                                                {
                                                                                                                                    if (flightDetails.Name.Equals("air:FlightDetails", StringComparison.OrdinalIgnoreCase) && flightDetails.Attributes["Key"].Value.Equals(airSegmentChild.Attributes["Key"].Value, StringComparison.Ordinal))
                                                                                                                                    {
                                                                                                                                        ...
                                                                                                                                        _journeyTime = BookingInfo.Attributes["TravelTime"].Value;
                                                                                ...
                                                                                if (bond.BoundType == "InBound" && bond.Legs.Count < 2)
                                                                                {
                                                                                    listOfInBound.Add(bond);
                                                                                    listOfBound = new ArrayList(listOfInBound);
                                                                                }
                                                                                else if (bond.BoundType == "OutBound" && bond.Legs.Count < 2)
                                                                                {
                                                                                    listOfOutBound.Add(bond);
                                                                                    listOfBound = new ArrayList(listOfOutBound);
                                                                                }
                                                                                ...
                                                                                foreach (GDSResModel.Leg leg_ in bond.Legs)
                                                                                {
                                                                                    if (leg_.AircraftCode.Equals(_BookingInfo.Attributes["SegmentRef"].Value, StringComparison.Ordinal))
                                                                                    {
                                                                                        leg_.FareClassOfService = _BookingInfo.Attributes["BookingCode"].Value;
                                                                                        leg_.AvailableSeat = _BookingInfo.Attributes["BookingCount"].Value;
                                                                                        leg_.Cabin = _BookingInfo.Attributes["CabinClass"].Value;
                                                                                        leg_.FareRulesKey = _BookingInfo.Attributes["FareInfoRef"].Value;
                                                                                        leg_.FareRulesValue = fareRuleInfo[_BookingInfo.Attributes["FareInfoRef"].Value];
                                                                                        if (baggageDetais != null && baggageDetais.ContainsKey(_BookingInfo.Attributes["FareInfoRef"].Value))
                                                                                        {
                                                                                            if (baggageDetais[_BookingInfo.Attributes["FareInfoRef"].Value].Split('|').Length == 3)
                                                                                            {
                                                                                                leg_.BaggageWeight = baggageDetais[_BookingInfo.Attributes["FareInfoRef"].Value].Split('|')[0];
                                                                                                leg_.BaggageUnit = baggageDetais[_BookingInfo.Attributes["FareInfoRef"].Value].Split('|')[1];
                                                                                                leg_.Branddesc = baggageDetais[_BookingInfo.Attributes["FareInfoRef"].Value].Split('|')[2].Split('\n')[0];
                                                                                                if (!IsDomestic)
                                                                                                    leg_.Remarks = baggageDetais[_BookingInfo.Attributes["FareInfoRef"].Value].Split('|')[2];
                                                                                            }
                                                                                        }
                                                                                        else
                                                                                        {
                                                            ...
                                                            if (listOfInBound.Count > 0 || listOfOutBound.Count > 0)
                                                            {
                                                                if (_airPricingpointCounter == airPricingpoint.ChildNodes.Count)
                                                                {
                                                                    seg = new GDSResModel.Segment();
                                                                    ...
                                                                    foreach (GDSResModel.Bond bond_ in listOfOutBound)
                                                                    {
                                                                        seg.Bonds.Add(bond_);
                                                                    }
                                                                    foreach (GDSResModel.Bond bond_ in listOfInBound)
                                                                    {
                                                                        seg.Bonds.Add(bond_);
                                                                    }
                                                                ...
                                                                bond = new GDSResModel.Bond();
                                                                listOfBound = new ArrayList();
                                                                listOfOutBound = new ArrayList();
                                                                listOfInBound = new ArrayList();
                                                                bond.Legs = new List<GDSResModel.Leg>();
                                                                contractId++;
                                                            }
                                                            #endregion
                                                    ...
                                                    //}
                                                    _airPricingpointCounter++;
                                                }// outer loop of airpriceinfo 3 times
                                            }
                                        }
                                        #endregion airPricingSolution
