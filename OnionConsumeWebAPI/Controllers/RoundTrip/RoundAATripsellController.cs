    ...
    public class RoundAATripsellController : Controller
    {
            ...
            for (int i = 0; i < dataArray.Length; i++)
            {
                ...
                if (!string.IsNullOrEmpty(tokenview) && dataArray[i].ToLower() == "airasia")
                {
                    ...
                    using (HttpClient client = new HttpClient())
                    {
                        ...
                        _ContactModel.sourceOrganization = "OTAIN";
                        _ContactModel.companyName = "JETWAYS TRAVELS PRIVATE LIMITED";
                        _ContactModel.distributionOption = "";
                ...
                if (!string.IsNullOrEmpty(tokenview) && dataArray[i].ToLower() == "akasaair")
                {
                    ...
                    using (HttpClient client = new HttpClient())
                    {
                        ...
                        if (responseAddContact.IsSuccessStatusCode)
                        {
                            var _responseAddContact = responseAddContact.Content.ReadAsStringAsync().Result;
                            Logs logs1 = new Logs();
                            logs1.WriteLogsR("Request: " + _ContactModel + "Url: " + AppUrlConstant.URLAkasaAir + "/api/nsk/v1/booking/contacts" + "\n Response: " + _responseAddContact, "5-Add_Contacts", "SameAkasaRT");
                ...
                using (HttpClient client = new HttpClient())
                {
                    if (!string.IsNullOrEmpty(tokenview) && dataArray[i1].ToLower() == "airasia")
                    {
                        ...
                        for (int i = 0; i < passengerdetails.Count; i++)
                        {
                            ...
                            {
                                string result = passengerdetails[i].passengerkey.Substring(0, passengerdetails[i].passengerkey.IndexOf('^')); 
                        ...
                        {                            
                            ...
                            if (passengerdetails[j].passengertypecode == "ADT" || passengerdetails[j].passengertypecode == "CHD")
                            {
                                 
                                continue;
                            }
                            if (passengerdetails[j].passengertypecode == "INFT" || passengerdetails[j].passengertypecode == "INF")
                            {
                                for (int k = 0; k < infantcount; k++)
                                {
                                   string result = passengerdetails[k].passengerkey.Substring(0, passengerdetails[k].passengerkey.IndexOf('^'));


                                    _PassengersModel1.nationality = "IN";
                                    _PassengersModel1.dateOfBirth = "2024-01-01";
                                    _PassengersModel1.residentCountry = "IN";
                                    _PassengersModel1.gender = "Male";

                                    InfantName nameINF = new InfantName();
                                    nameINF.first = passengerdetails[j].first;
                                    nameINF.middle = null;
                                    nameINF.last = passengerdetails[j].last;
                                    nameINF.title = null;
                                    nameINF.suffix = null;
                    ...
                    if (!string.IsNullOrEmpty(tokenview) && dataArray[i1].ToLower() == "akasaair")
                    {
                        ...
                        for (int i = 0; i < passengerdetails.Count; i++)
                        {
                            ...
                            if (passengerdetails[i].passengertypecode != null)
                            {
                                string resultAkasa = passengerdetails[i].passengerkey.Substring(0, passengerdetails[i].passengerkey.IndexOf('^'));
                                Name name = new Name();
                                _Info Info = new _Info();
                                if (passengerdetails[i].title == "Mr")
                                {
                                ...
                                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                HttpResponseMessage responsePassengers = await client.PutAsJsonAsync(AppUrlConstant.URLAkasaAir + "/api/nsk/v3/booking/passengers/" + resultAkasa, _PassengersModel);
                                if (responsePassengers.IsSuccessStatusCode)
                                {
                                    var _responsePassengers = responsePassengers.Content.ReadAsStringAsync().Result;
                                    Logs logs = new Logs();
                                    logs.WriteLogsR("Request: " + JsonConvert.SerializeObject(_PassengersModel) + "Url: " + AppUrlConstant.URLAkasaAir + "/api/nsk/v3/booking/passengers/" + passengerdetails[i].passengerkey + "\n Response: " + JsonConvert.SerializeObject(_responsePassengers), "6-Add_passenger_"+i, "SameAkasaRT");
                        ...
                        for (int i = 0; i < passengerdetails.Count; i++)
                        {
                            ...
                            if (passengerdetails[i].passengertypecode == "INFT" || passengerdetails[i].passengertypecode == "INF")
                            {
                                ...
                                for (int k = 0; k < infantcount; k++)
                                {
                                    string resultAkasainft = passengerdetails[k].passengerkey.Substring(0, passengerdetails[k].passengerkey.IndexOf('^'));
                                    ...
                                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                    HttpResponseMessage responsePassengers = await client.PostAsJsonAsync(AppUrlConstant.URLAkasaAir + "/api/nsk/v3/booking/passengers/" + resultAkasainft + "/infant", _PassengersModel1);
                                    if (responsePassengers.IsSuccessStatusCode)
                                    {
                                        var _responsePassengers = responsePassengers.Content.ReadAsStringAsync().Result;
                                        Logs logs = new Logs();
                                        logs.WriteLogsR("Request: " + _PassengersModel1 + "Url: " + AppUrlConstant.URLAkasaAir + "/api/nsk/v3/booking/passengers/" + passengerdetails[k].passengerkey + "/infant" + "\n Response: " + _responsePassengers, "7-Add_Infant"+k, "SameAkasaRT");
                                ...
                                if (responceGetBooking.IsSuccessStatusCode)
                                {
                                    var _responceGetBooking = responceGetBooking.Content.ReadAsStringAsync().Result;
                                    Logs logs = new Logs();
                                    //logs.WriteLogsR("Request: " + JsonConvert.SerializeObject("GetBookingRequest") + "Url: " + AppUrlConstant.URLAirasia + "/api/nsk/v1/booking" + "\n Response: " + JsonConvert.SerializeObject(_responceGetBooking), "GetBooking", "SameAkasaRT");
        ...
        public async Task<IActionResult> PostUnitkey(List<string> unitKey, List<string> ssrKey, List<string> BaggageSSrkey, List<string> FastfarwardAddon, List<string> PPBGAddon)
        {
                            ...
                            if (passenger != null)
                            {
                                ...
                                else if (passeengerKeyList.journeys[0].Airlinename.ToLower() == "airasia")
                                {
                                    ...
                                    for (int l1 = 0; l1 < ssrKey.Count; l1++)
                                    {
                                        ...
                                        int idx = 0;
                                        int paxnum = 0;
                                        //if (ssrKey[mealid].ToLower().Contains("rt0") || ssrKey[mealid].ToLower().Contains("rt1"))
                                            //_a++;
                                        if (mealid < ssrKey.Count)
                                        {
                                            ...
                                            if (ssrKey[mealid].ToLower().Contains("airasia")  && (ssrKey[mealid].ToLower().Contains("oneway0") || ssrKey[mealid].ToLower().Contains("oneway1")))
                                            {
                                                ...
                                                if (!string.IsNullOrEmpty(token))
                                                {
                                                    using (HttpClient client = new HttpClient())
                                                    {
                                                        ...
                                                        logs1.WriteLogsR(jsonSellSSR, "10-SellMeal_Req_"+ (l1+1), "SameAirAsiaRT");
                                                        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                                        ...
                                                        if (responseSellSSR.IsSuccessStatusCode)
                                                        {
                                                            ...
                                                            logs1.WriteLogsR(_responseresponseSellSSR, "10-SellMeal_Res_"+ (l1+1), "SameAirAsiaRT");
                                            ...
                                            else if (ssrKey[mealid].ToLower().Contains("airasia")  && (ssrKey[mealid].ToLower().Contains("rt0") || ssrKey[mealid].ToLower().Contains("rt1")))
                                            {
                                                if (ssrKey[mealid].Length > 1)
                                                {
                                                    ssrsubKey2 = ssrKey[mealid].Split('_');
                                                ...
                                                if (!string.IsNullOrEmpty(token))
                                                {
                                                    using (HttpClient client = new HttpClient())
                                                    {
                                                        ...
                                                        _sellSSRModel.currencyCode = "INR";
                                                        var jsonSellSSR = JsonConvert.SerializeObject(_sellSSRModel, Formatting.Indented);
                                                        logs1.WriteLogsR(jsonSellSSR, "10-SellMealRound_Req_"+ (l1+1), "SameAirAsiaRT");
                                                        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                                        HttpResponseMessage responseSellSSR = await client.PostAsJsonAsync(AppUrlConstant.URLAirasia + "/api/nsk/v2/booking/ssrs/" + mealskey, _sellSSRModel);
                                                        if (responseSellSSR.IsSuccessStatusCode)
                                                        {
                                                            var _responseresponseSellSSR = responseSellSSR.Content.ReadAsStringAsync().Result;
                                                            logs1.WriteLogsR(_responseresponseSellSSR, "10-SellMealRound_Res_"+ (l1+1), "SameAirAsiaRT");
                                    ...
                                    for (int b = 0; b < BaggageSSrkey.Count; b++)
                                    {
                                        ...
                                        if (bagid < BaggageSSrkey.Count)
                                        {
                                            ...
                                            if (BaggageSSrkey[bagid].ToLower().Contains("airasia") &&  _a == 0 &&(BaggageSSrkey[bagid].ToLower().Contains("oneway0") || BaggageSSrkey[bagid].ToLower().Contains("oneway1")))
                                            {
                                                ...
                                                if (!string.IsNullOrEmpty(token))
                                                {
                                                    using (HttpClient client = new HttpClient())
                                                    {
                                                        SellSSRModel _sellSSRModel = new SellSSRModel();
                                                        _sellSSRModel.count = 1;
                                                        _sellSSRModel.note = "bagssr";
                                                        _sellSSRModel.forceWaveOnSell = false;
                                                        _sellSSRModel.currencyCode = "INR";
                                                        var jsonSellSSR = JsonConvert.SerializeObject(_sellSSRModel, Formatting.Indented);
                                                        logs1.WriteLogsR(jsonSellSSR, "9-SellBaggage_Req_"+(b+1), "SameAirAsiaRT");
                                                        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                                        ...
                                                        if (responseSellSSR.IsSuccessStatusCode)
                                                        {
                                                            ...
                                                            var _responseresponseSellSSR = responseSellSSR.Content.ReadAsStringAsync().Result;
                                                            logs1.WriteLogsR(_responseresponseSellSSR, "9-SellBaggage_Res_"+(b+1), "SameAirAsiaRT");
                                            ...
                                            else if (BaggageSSrkey[bagid].ToLower().Contains("airasia") && _a == 0 && (BaggageSSrkey[bagid].ToLower().Contains("rt0") || BaggageSSrkey[bagid].ToLower().Contains("rt1")))
                                            {
                                                if (BaggageSSrkey[bagid].Length > 1)
                                                {
                                                    ssrsubKey2 = BaggageSSrkey[bagid].Split('_');
                                                ...
                                                if (!string.IsNullOrEmpty(token))
                                                {
                                                    using (HttpClient client = new HttpClient())
                                                    {
                                                        ...
                                                        _sellSSRModel.currencyCode = "INR";
                                                        var jsonSellSSR = JsonConvert.SerializeObject(_sellSSRModel, Formatting.Indented);
                                                        logs1.WriteLogsR(jsonSellSSR, "9-SellBaggageRound_Req_"+(b+1), "SameAirAsiaRT");
                                                        client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                                        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                                                        ...
                                                        {
                                                            var _responseresponseSellSSR = responseSellSSR.Content.ReadAsStringAsync().Result;
                                                            logs1.WriteLogsR( _responseresponseSellSSR, "9-SellBaggageRound_Res"+(b+1), "SameAirAsiaRT");
                                ...
                                else if (passeengerKeyList.journeys[0].Airlinename.ToLower() == "akasaair")
                                {
                                                ...
                                                if (!string.IsNullOrEmpty(token))
                                                {
                                                    using (HttpClient client = new HttpClient())
                                                    {
                                                        ...
                                                        if (responseSellSSR.IsSuccessStatusCode)
                                                        {
                                                            ...
                                                            var _responseresponseSellSSR = responseSellSSR.Content.ReadAsStringAsync().Result;
                                                            logs1.WriteLogsR("Request: " + jsonSellSSR + "Url: " + AppUrlConstant.URLAkasaAir + "/api/nsk/v2/booking/ssrs/" + mealskey + "\n Response: " + _responseresponseSellSSR, "9-Assignmeal", "SameAkasaRT");
                                                            var JsonObjresponseresponseSellSSR = JsonConvert.DeserializeObject<dynamic>(_responseresponseSellSSR);
                                                        }
                                                ...
                                                if (!string.IsNullOrEmpty(token))
                                                {
                                                    using (HttpClient client = new HttpClient())
                                                    {
                                                       ...
                                                       // _sellSSRModel.ssrSellMode = 2;
                                                        ...
                                                        {
                                                            var _responseresponseSellSSR = responseSellSSR.Content.ReadAsStringAsync().Result;
                                                            logs1.WriteLogsR("Request: " + jsonSellSSR + "Url: " + AppUrlConstant.URLAkasaAir + "/api/nsk/v2/booking/ssrs/" + mealskey + "\n Response: " + _responseresponseSellSSR, "9-AssignmealReturn", "SameAkasaRT");
                                                            var JsonObjresponseresponseSellSSR = JsonConvert.DeserializeObject<dynamic>(_responseresponseSellSSR);
                                                        }
                                    ...
                                    for (int b = 0; b < BaggageSSrkey.Count; b++)
                                    {
                                        ...
                                        if (bagid < BaggageSSrkey.Count)
                                        {
                                                ...
                                                if (!string.IsNullOrEmpty(token))
                                                {
                                                    using (HttpClient client = new HttpClient())
                                                    {
                                                        ...
                                                        if (responseSellSSR.IsSuccessStatusCode)
                                                        {
                                                            ...
                                                            var _responseresponseSellSSR = responseSellSSR.Content.ReadAsStringAsync().Result;
                                                            logs1.WriteLogsR("Request: " + jsonSellSSR + "Url: " + AppUrlConstant.URLAkasaAir + "/api/nsk/v2/booking/ssrs/" + bagskey + "\n Response: " + _responseresponseSellSSR, "12-AssignBaggage", "SameAkasaRT");
                                                            var JsonObjresponseresponseSellSSR = JsonConvert.DeserializeObject<dynamic>(_responseresponseSellSSR);
                                                        }
                                            ...
                                            else if (BaggageSSrkey[bagid].ToLower().Contains("akasaair") && _a == 0 && (BaggageSSrkey[bagid].ToLower().Contains("rt0") || BaggageSSrkey[bagid].ToLower().Contains("rt1")))
                                            {
                                                if (BaggageSSrkey[bagid].Length > 1)
                                                {
                                                    ssrsubKey2 = BaggageSSrkey[bagid].Split('_');
                                                ...
                                                if (!string.IsNullOrEmpty(token))
                                                {
                                                    using (HttpClient client = new HttpClient())
                                                    {
                                                        ...
                                                        {
                                                            var _responseresponseSellSSR = responseSellSSR.Content.ReadAsStringAsync().Result;
                                                            logs1.WriteLogsR("Request: " + jsonSellSSR + "Url: " + AppUrlConstant.URLAkasaAir + "/api/nsk/v2/booking/ssrs/" + bagskeyCon + "\n Response: " + _responseresponseSellSSR, "12-AssignBaggageReturn", "SameAkasaRT");
                                                            var JsonObjresponseresponseSellSSR = JsonConvert.DeserializeObject<dynamic>(_responseresponseSellSSR);
                                                        }
                                    ...
                                    for (int i = 0; i < journeyscount; i++)
                                    {
                                        ...
                                        for (int l2 = 0; l2 < _obj.SSRcodeOneWayI.Count; l2++)
                                        {
                                            ...
                                            using (HttpClient client = new HttpClient())
                                            {
                                                ...
                                                {
                                                    var _responseSeatAssignment = responceSeatAssignment.Content.ReadAsStringAsync().Result;
                                                    logs1.WriteLogsR("Request: " + jsonSeatAssignmentRequest + "Url: " + AppUrlConstant.URLAkasaAir + "/api/nsk/v2/booking/passengers/" + passengerkey + "/seats/" + pas_unitKey + "\n Response: " + _responseSeatAssignment, "11-AssignSeat", "SameAkasaRT");
                                                    var JsonObjSeatAssignment = JsonConvert.DeserializeObject<dynamic>(_responseSeatAssignment);
                                                }
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                logs1.WriteLogs(GetbookingPnrRequest, "13-GetBookingBalanceDueReq", "SameAirAsiaRT");
                client.DefaultRequestHeaders.Accept.Add(new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/json"));
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
                HttpResponseMessage GetBookingBalanceDue = await client.GetAsync(AppUrlConstant.AirasiaGetBoking);
                if (GetBookingBalanceDue.IsSuccessStatusCode)
                {
                    // string BaseURL1 = "http://localhost:5225/";
                    var _responceGetBooking = GetBookingBalanceDue.Content.ReadAsStringAsync().Result;
                    logs1.WriteLogs(_responceGetBooking, "13-GetBookingBalanceDueRes", "SameAirAsiaRT");
                    //logs1.WriteLogs("Url: " + AppUrlConstant.AirasiaGetBoking + "\n Response: " + JsonConvert.SerializeObject(_responceGetBooking), "GetBooking", "AirAsiaOneWay");
